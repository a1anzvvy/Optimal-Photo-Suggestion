...
def Inception_layer(input_data,train_flag=False):    
    ...
    with tf.variable_scope('Inception_layer'):
        ...
        one_by_one_3_conv = tf.layers.conv2d(input_data, filters=32, kernel_size=[1,1], padding='same',activation=None,name='one_by_one_3')
        one_by_one_3_batch = tf.layers.batch_normalization(one_by_one_3_conv,center=True,scale=True,epsilon=0.00001,training=train_flag)
        one_by_one_3 = tf.nn.relu(one_by_one_3_batch)
        # 3x3 
        three_by_three_1_conv= tf.layers.conv2d(one_by_one_1, filters=32, kernel_size=[3,3], padding='same',activation=None,name='three_by_three_1')
        three_by_three_1_batch = tf.layers.batch_normalization(three_by_three_1_conv,center=True,scale=True,epsilon=0.00001,training=train_flag)
        three_by_three_1 = tf.nn.relu(three_by_three_1_batch)
        ...
        three_by_three_3_conv = tf.layers.conv2d(three_by_three_2, filters=32, kernel_size=[3,3], padding='same',activation=None,name='three_by_three_3')
        three_by_three_3_batch = tf.layers.batch_normalization(three_by_three_3_conv,center=True,scale=True,epsilon=0.00001,training=train_flag)
        three_by_three_3 = tf.nn.relu(three_by_three_3_batch)
        # max pooling
...
def model(input_data,train_flag = False):
    ...
    # 4.16 Modified by wenyuan
    with tf.variable_scope('fc1'):
        fc_layer_1=tf.layers.dense(total_input,units=512,activation=None,name='fc1')
        #fc_layer_1=tf.nn.dropout(fc_layer_1,keep_prob=DROPOUT_PROB)
        fc_layer_batch = tf.layers.batch_normalization(fc_layer_1,center=True,scale=True,epsilon=0.001,training=train_flag)
        fc_layer_1_out = tf.nn.tanh(fc_layer_batch)

    #print (total_input.shape)
    with tf.variable_scope('fc2'):
        fc_layer_2=tf.layers.dense(fc_layer_1_out,units=2,name='fc2')
        
    return fc_layer_2

# 4.16 Added by wenyuan
    '''
...
def main(argv=None):
    ...
    pred = model(input_data) #used in training
    #pred_tv = model(input_data, train_flag = False) #used for testing and validation
    '''
    # 4.16 Added by wenyuan
...
#                    result = []
#                    for t in range((K_FOLD - 1) * SUBSAMPLE_NUM):
#                        feed_dict={input_data:train_data[t].reshape((1,INPUT_HEIGHT,INPUT_WIDTH,INPUT_CHANNEL))}
#                        classification = sess.run(pred, feed_dict)
#                        result.extend(classification.argmax(axis=1))
#                    label_pos=np.argmax(label_train,axis=1) 
#                    boolResult = np.array(result)==np.array(label_pos)[0:]
#                    train_accu = len(boolResult[boolResult==True])/((K_FOLD - 1) * SUBSAMPLE_NUM)
#                    print("cnm",boolResult[0:100])
#                    print (train_accu)
#                    train_accur.append(train_accu)
...
#            test_accu = len(boolResult[boolResult==True])/SUBSAMPLE_NUM
#            print (test_accu)
...
#    
    ...
    saver=tf.train.Saver()
    saved_episode=[120,130,140,150]
    
    with tf.Session() as sess:
        for index in saved_episode:
            accuracy=[]
            myMean = []
            result=[]
            saver.restore(sess, "./modelWeights/newWeights_dropout/weights_At"+str(index)+"K_FOLD1"+".ckpt")
            ...
            for i in range(4000,5000):
                ...
                if i ==0 : print("classification",classification)
                result.extend(classification.argmax(axis=1))
